...
public class LoginUser {
    ...
    public String authenticate(String inputAccount, String inputPassword)
        ...
        /*
            ...
            다만 에러가 발생하면 프로그램이 튈기기 때문에
            그에 따른 예외 처리는 호출하는 쪽에게 떠넘긴다.
        ...
        if (!this.account.equals(inputAccount)) {
            ...
            // throw(raise): 유발시키다.
            throw new LoginInvalidException("회원가입 되지 않은 계정입니다.");
        }
        if (!this.password.equals(inputPassword)) {
            // 사용자에게 등록되지 않은 비밀번호임을 알림
            throw new LoginInvalidException("비밀번호가 틀렸습니다.");
        }
        /*
            실제로 인증 로직에서 로그인에 성공하면
            브라우저나 클라이언트에게 인증 토큰을 포함한 인증 정보를 보냄.
         */
